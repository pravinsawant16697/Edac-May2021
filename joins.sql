...
select last_name, first_name, salary from employees where salary > (select salary from employees where last_name='Bull');
...
select last_name, first_name from employees where manager_id!=0 and manager_id in
...
# Write a query to find the name (first_name, last_name), and salary of the employees whose salary is equal to
...
where countries.country_id = locations.country_id and departments.location_id = locations.location_id;
...
SELECT  emp.employee_id as Employee_ID, emp.last_name as Employee_Name, mgr.employee_id as Manager_id, mgr.last_name as Manager_Name
...
#11 Write a query to display job title, employee name, 
...
as diffsal from employees, jobs
where employees.job_id = jobs.job_id;

#12 Write a query to display the job history that were done 
